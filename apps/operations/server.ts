...
async function main() {
  ...
  // ── 3. Initialise all modules (seed data, indexes) ────────────────────────
  ...
  // ── 6. Order view helper (used in multiple routes) ────────────────────────
  const orderView = async (id, tenant) => {
    const order = await db.prepare(
      'SELECT * FROM orders WHERE id=? AND business_id=? AND branch_id=?'
    ).get(id, tenant.businessId, tenant.branchId)
    if (!order) return null
    const items = await db.prepare(
      'SELECT menu_item_id AS "menuItemId", name, unit_price AS "unitPrice", quantity FROM order_items WHERE order_id=? AND business_id=?'
    ).all(id, tenant.businessId)
    return { ...order, items }
  }
  ...
  createServer(async (req, res) => {
    try {
      ...
      // POST /api/upload/products  → product photo → R2
      // POST /api/upload/logos     → business logo → R2
      // POST /api/upload/documents → document      → R2
      ...
      if (uploadMatch && req.method === 'POST') {
        ...
        const businessId = authUser.businessId ?? 'platform'
        const handled = await handleUpload(req, res, folder, businessId)
        if (handled) return
      }
      ...
      if (req.method === 'GET' && u.pathname === '/api/menu')
        return send(res, 200, await db.prepare(
          'SELECT id,name,category AS cat,price,favorite AS fav,image FROM menu_items WHERE business_id=? AND available=true ORDER BY id'
        ).all(tenant.businessId))

      if (req.method === 'GET' && u.pathname === '/api/menu/admin')
        return send(res, 200, await db.prepare(
          'SELECT id,name,category,price,favorite,image,available FROM menu_items WHERE business_id=? ORDER BY category,name'
        ).all(tenant.businessId))

      if (req.method === 'GET' && u.pathname === '/api/categories')
        return send(res, 200, await db.prepare(`
          SELECT c.*,COUNT(m.id) "itemCount",
            SUM(CASE WHEN m.available THEN 1 ELSE 0 END) "availableCount",
            COALESCE(SUM(m.price),0) "catalogValue"
          FROM categories c
          LEFT JOIN menu_items m ON m.category=c.key AND m.business_id=c.business_id
      ...
      if (req.method === 'POST' && u.pathname === '/api/categories') {
        ...
        const key = String(input.key || name).trim().toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '')
        if (!name || !key) return send(res, 400, { error: 'Category name is required' })
        try {
          const result = await db.prepare(
            'INSERT INTO categories(key,name,description,is_active,created_at,business_id) VALUES(?,?,?,true,NOW(),?) RETURNING id'
          ).run(key, name, input.description || '', tenant.businessId)
          return send(res, 201, { id: result.lastInsertRowid, key })
        } catch {
          return send(res, 409, { error: 'A category with this name or key already exists' })
        }
      }
      ...
      if (req.method === 'PATCH' && categoryItem) {
        const input = await body(req)
        const current = await db.prepare('SELECT * FROM categories WHERE id=? AND business_id=?').get(+categoryItem[1], tenant.businessId)
        if (!current) return send(res, 404, { error: 'Category not found' })
        await db.prepare('UPDATE categories SET name=?,description=?,is_active=? WHERE id=? AND business_id=?')
          .run(input.name ?? current.name, input.description ?? current.description,
               input.isActive === undefined ? current.is_active : Boolean(input.isActive),
               current.id, tenant.businessId)
        return send(res, 200, { ok: true })
      }

      // ── Menu item (legacy POS) ─────────────────────────────────────────────
      if (req.method === 'POST' && u.pathname === '/api/menu') {
        const input = await body(req)
        const price = Math.trunc(Number(input.price))
        if (!input.name || !input.category || !Number.isFinite(price) || price < 0)
          return send(res, 400, { error: 'Name, category and a valid price are required' })
        ...
        const warehouse = await db.prepare(
          'SELECT id FROM warehouses WHERE business_id=? AND branch_id=? ORDER BY created_at LIMIT 1'
        ).get(tenant.businessId, tenant.branchId)
        // Handle image: if base64, upload to R2 first
        let imageUrl = input.image || null
        if (imageUrl && imageUrl.startsWith('data:')) {
          imageUrl = await uploadToR2(imageUrl, 'products', tenant.businessId) || null
        }
        await db.prepare(
          'INSERT INTO menu_items(id,name,category,price,favorite,image,available,business_id,warehouse_id) VALUES(?,?,?,?,?,?,true,?,?)'
        ).run(id, input.name.trim(), input.category.trim().toLowerCase(), price, input.favorite ? true : false, imageUrl, tenant.businessId, warehouse?.id || null)
      ...
      if (req.method === 'PATCH' && menuItem) {
        const input = await body(req)
        const item = await db.prepare('SELECT * FROM menu_items WHERE id=? AND business_id=?').get(+menuItem[1], tenant.businessId)
        ...
        if (imageUrl && imageUrl.startsWith('data:')) {
          imageUrl = await uploadToR2(imageUrl, 'products', tenant.businessId) || item.image
        }
        await db.prepare(
          'UPDATE menu_items SET name=?,category=?,price=?,favorite=?,image=?,available=? WHERE id=? AND business_id=?'
        ).run(input.name ?? item.name, input.category ?? item.category, price,
              input.favorite === undefined ? item.favorite : Boolean(input.favorite),
              imageUrl,
              input.available === undefined ? item.available : Boolean(input.available),
              item.id, tenant.businessId)
        return send(res, 200, { ok: true })
      }
      ...
      if (req.method === 'GET' && u.pathname === '/api/orders')
        return send(res, 200, await db.prepare(
          'SELECT * FROM orders WHERE business_id=? AND branch_id=? ORDER BY created_at DESC LIMIT 50'
        ).all(tenant.businessId, tenant.branchId))
      ...
      if (req.method === 'PATCH' && orderRecord) {
        const input = await body(req)
        const allowed = ['OPEN','CONFIRMED','SHIPPED','DELIVERED','CANCELLED','HELD']
        ...
        if (current.status === 'PAID' && status === 'CANCELLED')
          return send(res, 409, { error: 'A paid order must be returned or refunded, not cancelled' })
        await db.prepare('UPDATE orders SET status=? WHERE id=? AND business_id=? AND branch_id=?')
          .run(status, current.id, tenant.businessId, tenant.branchId)
      ...
      if (req.method === 'POST' && u.pathname === '/api/orders') {
        ...
        const ids = input.items.map(x => x.menuItemId)
        const menu = await db.prepare(
          `SELECT * FROM menu_items WHERE business_id=? AND id IN (${ids.map(() => '?').join(',')}) AND available=true`
        ).all(tenant.businessId, ...ids)
        if (menu.length !== new Set(ids).size) return send(res, 400, { error: 'One or more items are unavailable' })
        const lines = input.items.map(x => ({ ...menu.find(m => m.id === x.menuItemId), quantity: Math.max(1, Math.trunc(x.quantity)) }))
        const subtotal = lines.reduce((n, x) => n + x.price * x.quantity, 0)
        const discount = Math.min(Math.max(0, Math.trunc(input.discount || 0)), subtotal)
        ...
        await db.transaction(async tx => {
          await tx.prepare('INSERT INTO orders(id,order_number,type,table_name,guests,status,subtotal,discount,tax,total,created_at,paid_at,inventory_deducted,business_id,branch_id) VALUES(?,?,?,?,?,?,?,?,0,?,NOW(),NULL,false,?,?)').run(id, number, input.type || 'Dine in', input.tableName || null, input.guests || 1, 'OPEN', subtotal, discount, total, tenant.businessId, tenant.branchId)
          for (const x of lines) {
            await tx.prepare('INSERT INTO order_items(order_id,menu_item_id,name,unit_price,quantity,business_id) VALUES(?,?,?,?,?,?)').run(id, x.id, x.name, x.price, x.quantity, tenant.businessId)
          }
        })
      ...
      if (req.method === 'POST' && submit) {
        ...
        if (!order || !['OPEN', 'KITCHEN_PENDING'].includes(order.status))
          return send(res, 409, { error: 'Order cannot be submitted' })
      ...
      if (req.method === 'POST' && payment) {
        const input = await body(req)
        const order = await orderView(payment[1], tenant)
        if (!order) return send(res, 404, { error: 'Order not found' })
        if (order.paid_at || order.status === 'CREDIT')
          return send(res, 409, { error: 'Order is already settled or assigned to credit' })
        if (!['Cash', 'Card', 'Mobile money', 'Credit'].includes(input.method))
          return send(res, 400, { error: 'Invalid payment method' })
        if (input.method === 'Credit' && !String(input.borrowerName || '').trim())
          return send(res, 400, { error: 'Customer name is required for a credit sale' })
        ...
        if (input.method === 'Credit') {
          ...
          if (Number.isNaN(due.getTime()) || due < new Date(new Date().toDateString()))
            return send(res, 400, { error: 'A valid future due date is required' })
      ...
      if (req.method === 'GET' && u.pathname === '/api/sales') {
        ...
        const revenue = paid.reduce((n, x) => n + x.total, 0)
      ...
      if (req.method === 'GET' && u.pathname === '/api/dashboard/analytics')
        return send(res, 200, await businessAnalytics(db, tenant))
  ...
  }).listen(port, '0.0.0.0', () => console.log(`✅ Zentro server running at http://0.0.0.0:${port}`))
}
